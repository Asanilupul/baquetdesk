...
class BackupService
{
    ...
    /**
     * @return array{path: string, filename: string, size: int, tables: int, created_at: string, format: string}
     */
    public function createBackup(string $trigger = 'manual'): array
    {
        $dir = $this->backupDirectory();
        if (! is_dir($dir) || ! is_writable($dir)) {
            throw new \RuntimeException('Backup directory not writable: '.$dir.'. Create /home/alphatra/banquetdesk_backups and set permissions 775.');
        }
        ...
        try {
            ...
            if (config('database.default') === 'sqlite') {
                ...
                if (is_string($dbPath) && is_file($dbPath)) {
                    @copy($dbPath, $tempDir.'/database.sqlite');
                }
            }

            $archive = $this->buildArchive($dir, $stamp, $trigger, $tempDir, $fullJson);
            $size = is_file($archive['path']) ? (int) filesize($archive['path']) : 0;
            $this->rememberLastBackup($archive['filename'], $archive['path'], $size, $trigger, null);
            ...
            $this->rememberLastBackup(null, null, 0, $trigger, $e->getMessage());
            throw $e;
        } finally {
            if (is_dir($tempDir)) {
     ...
     */
    private function buildArchive(string $dir, string $stamp, string $trigger, string $tempDir, string $fullJson): array
    {
        // 1) Preferred: ZipArchive
        if (class_exists(ZipArchive::class)) {
            $filename = "banquetdesk_backup_{$stamp}_{$trigger}.zip";
        ...
        if (class_exists(PharData::class)) {
            $filename = "banquetdesk_backup_{$stamp}_{$trigger}.tar";
        ...
        if (function_exists('gzencode')) {
            $filename = "banquetdesk_backup_{$stamp}_{$trigger}.json.gz";
            $path = $dir.'/'.$filename;
        ...
        $filename = "banquetdesk_backup_{$stamp}_{$trigger}.json";
        $path = $dir.'/'.$filename;
        File::put($path, $fullJson);
        if (! is_file($path)) {
            throw new \RuntimeException('Unable to create backup file. Enable PHP zip extension in cPanel MultiPHP INI, or ensure backup folder is writable.');
        }
    ...
    public function status(): array
    {
        ...
        return [
            ...
            'last_run' => $settings['backup_last_run'] ?? null,
            'last_file' => $settings['backup_last_file'] ?? null,
            'last_size' => isset($settings['backup_last_size']) ? (int) $settings['backup_last_size'] : null,
            'last_error' => $settings['backup_last_error'] ?? null,
            'last_trigger' => $settings['backup_last_trigger'] ?? null,
            'files' => $this->listBackups(10),
            'zip_available' => class_exists(ZipArchive::class),
            'restore_formats' => ['.zip', '.tar', '.tar.gz', '.json.gz', '.json'],
            'cron_hint' => '0 2 * * * /usr/local/bin/ea-php83 '.base_path().'/artisan backup:daily >> /home/alphatra/banquetdesk_backups/cron.log 2>&1',
            'schedule_hint' => '* * * * * /usr/local/bin/ea-php83 '.base_path().'/artisan schedule:run >> /dev/null 2>&1',
        ];
    }

    /**
     * Emergency restore: replace live table data from an uploaded BanquetDesk backup.
     *
     * @return array{tables_restored: int, rows_restored: int, safety_backup: ?string, source: string, warnings: list<string>}
     */
    public function restoreFromFile(string $absolutePath, string $originalName = ''): array
    {
        ...
        $safetyName = null;
        try {
            $safety = $this->createBackup('pre-restore');
        ...
        try {
            DB::transaction(function () use ($payload, $allowed, &$tablesRestored, &$rowsRestored, &$warnings) {
                foreach ($this->backupTables() as $table) {
    ...
    /**
     * Restore from a backup file already stored in the server backup folder.
     *
     * @return array{tables_restored: int, rows_restored: int, safety_backup: ?string, source: string, warnings: list<string>}
     */
    public function restoreFromServerFilename(string $filename): array
    {
        $filename = basename(str_replace(["\0", '\\', '/'], '', $filename));
        if ($filename === '' || ! str_starts_with($filename, 'banquetdesk_backup_')) {
            throw new \RuntimeException('Invalid server backup filename.');
        }
        $path = $this->backupDirectory().'/'.$filename;
        if (! is_file($path)) {
            throw new \RuntimeException('Server backup file not found: '.$filename);
        }

        return $this->restoreFromFile($path, $filename);
    }
    ...
    private function rememberLastBackup(?string $filename, ?string $path, int $size, string $trigger, ?string $error): void
    {
        $pairs = [
            'backup_last_run' => now()->toDateTimeString(),
            'backup_last_file' => $filename ?: '',
            'backup_last_size' => (string) $size,
            'backup_last_path' => $path ?: '',
            'backup_last_trigger' => $trigger,
