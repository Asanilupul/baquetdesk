...
(function (global) {
  ...
  /** Legacy combo (price_mode / single menu_id) → menu_options[] so one pricing path. */
  function normalizeCombo(combo) {
    if (!combo) return combo;
    let options = Array.isArray(combo.menu_options) ? combo.menu_options.map(normalizeOption) : [];
    if (!options.length && (combo.menu_id || num(combo.price, 0) > 0)) {
      let tierTotal = num(combo.price, 0);
      if (combo.price_mode === "per_person" && tierTotal > 0) {
        const paxHint = Math.max(50, parseInt(combo.pax_count, 10) || 100);
        tierTotal = tierTotal * paxHint;
      }
      const tiers = emptyTiers();
      TIERS.forEach((p) => {
      ...
      options = [
        normalizeOption({
          ...
          extra_plate_price: num(combo.menu_unit_price, 0),
          tiers,
  ...
  function buildBill(option, pax, optionalExtras, catalogs) {
    const opt = normalizeOption(option || {});
    const calc = calculate(opt, pax);
    const included = includedLabels(opt, catalogs);
    const optLines = [];
    let optionalTotal = 0;
    ...
    if (calc.ok) {
      lines.push({
        ...
        description: `${opt.menu_name || "Menu"} — ${calc.anchor} pax package`,
        quantity: 1,
        rate: calc.tierPrice,
    ...
    return {
      ...
      snapshot: {
        version: 1,
        combo_menu_option_id: opt.id || null,
        menu_id: opt.menu_id || null,
        menu_name: opt.menu_name || "",
        pax: calc.pax,
        anchor: calc.anchor,
